...
import { ProjectIdea, SavedProjectIdea, TaskPriority, User } from '../../types';
...
const ProjectIdeasPage: React.FC = () => {
  ...
  const [ideaForProject, setIdeaForProject] = useState<Omit<ProjectIdea, 'organizationId' | 'id'> & {id?:string} | null>(null);
  const [ideaForTask, setIdeaForTask] = useState<Omit<ProjectIdea, 'organizationId' | 'id'> & {id?:string} | null>(null);
  ...
  const handleGenerateIdeas = async () => {
    ...
    try {
      ...
      const parsedData = await generateProjectIdeas(prompt);
  ...
  const handleSaveIdea = (idea: Omit<ProjectIdea, 'organizationId' | 'id'> & {id?:string}) => {
    saveProjectIdea(idea); // AppContext.saveProjectIdea will handle orgId
  };
  ...
  const handleCopyIdea = (idea: ProjectIdea | Omit<ProjectIdea, 'organizationId' | 'id'>) => {
  ...
  const handleCreateProjectFromIdea = (idea: Omit<ProjectIdea, 'organizationId' | 'id'> & {id?:string}) => {
    setIdeaForProject(idea);
    setIsProjectFormModalOpen(true);
  };

  const handleAddIdeaAsTask = (idea: Omit<ProjectIdea, 'organizationId' | 'id'> & {id?:string}) => {
    setIdeaForTask(idea);
  ...
  const renderIdeaCard = (idea: ProjectIdea | (Omit<ProjectIdea, 'organizationId' | 'id'> & {id: string}), isSavedContext: boolean) => (
      ...
      {activeTab === 'generate' && (
        <div role="tabpanel" aria-labelledby="generate-tab">
          <div className="bg-white p-6 rounded-lg shadow-md mb-8">
            <Textarea
              id="ideaTopic"
              label={t('projectIdeaGeneratorInputLabel')}
              value={topic}
