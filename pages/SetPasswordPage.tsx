...
const SetPasswordPage: React.FC = () => {
  const { verifyInvitationToken, setUserPasswordAndActivate } = useAppContext();
  ...
  useEffect(() => {
    ...
    const verifiedUser = verifyInvitationToken(token);
    if (verifiedUser) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!user) {
      setError(t('setPasswordFailedError')); 
      return;
    }

    setIsLoading(true);
    const success = setUserPasswordAndActivate(user.id, password);
    setIsLoading(false);
  ...
  const pageContent = () => {
    if (isValidToken === null) { 
        return <p className="text-center text-gray-700">{t('loadingOrNotFound')}</p>;
    }
    ...
    if (!isValidToken || !user) {
        return (
            ...
            <Link to="/register" className="block text-center font-medium text-primary hover:text-primary-dark hover:underline">
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            <p className="text-sm text-gray-700 mb-1">{t('setPasswordInstructions', { email: user.email })}</p>
            <Input
