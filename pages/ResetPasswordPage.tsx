...
const ResetPasswordPage: React.FC = () => {
  const { verifyPasswordResetToken, resetUserPassword } = useAppContext();
  ...
  useEffect(() => {
    ...
    const verifiedUser = verifyPasswordResetToken(token);
    if (verifiedUser) {
  ...
  }, [token, verifyPasswordResetToken, t]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setIsLoading(true);
    const success = resetUserPassword(user.id, password);
    setIsLoading(false);
