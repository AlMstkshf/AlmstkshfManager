...
import { formatDate, getTaskStatusColor } from '../../utils/helpers'; // Reusing getTaskStatusColor for bar color
    ...
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <div className="space-y-4">
        {sortedTasks.map(task => {
          const statusColor = getTaskStatusColor(task.status).split(' ')[0]; // Get bg color part
            ...
            <div key={task.id} className={`p-4 rounded-md border ${statusColor.replace('bg-', 'border-').replace('-500', '-300')} shadow-sm`}>
              <div className="flex justify-between items-center mb-2">
                ...
                <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${getTaskStatusColor(task.status)}`}>
                  {t(`taskStatus${task.status.replace(/\s/g, '')}` as any)}
                </span>
              </div>
