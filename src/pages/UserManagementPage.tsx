...
import UserForm from @/componen@/use@/UserForm';
  ...
  return (
    <div className="container mx-auto py-6 px-4 space-y-8">
      <div className="flex justify-between items-center">
        ...
        {isCurrentUserAdmin() && (
            <Button onClick={openAddUserModal} leftIcon={<PlusIco@/>}>
            {t('addNewUser')}
           @/Button>
        )}
     @/div>
      ...
      {isCurrentUserAdmin() && organization && (
        <Card title={t('organizationSettings')}>
                ...
                {isEditingOrgName ? (
                   ...
                   @/div>
                    ...
                    <>
                       ...
                       @/p>
                        <Button onClick={() => setIsEditingOrgName(true)} variant="ghost" size="sm" leftIcon={<EditIc@/>}>
                           {t('editTaskBtn')}
                       @/Button>
                   @/>
                )}
                ...
                <tr key={user.id}>
                 ...
                 @/td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2 rtl:space-x-reverse">
                    {isCurrentUserAdmin() && (
                        <>
                        <Button variant="ghost" size="sm" onClick={() => openEditUserModal(user)} leftIcon={<EditIco@/>}>
                            {t('editTaskBtn')}
                       @/Button>
                        {user.id !== currentUser?.id && (
                          <Button variant="ghost" size="sm" onClick={() => openDeleteConfirmModal(user)} className="text-red-600 hover:bg-red-100" leftIcon={<DeleteIco@/>}>
                              {t('deleteUserBtn')}
                         @/Button>
                        )}
                       @/>
