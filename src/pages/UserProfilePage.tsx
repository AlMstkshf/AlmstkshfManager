...
const UserProfilePage: React.FC = () => {
  ...
  const handleEditTaskRedirect = (task: Task) => {
    ...
    if (project) {
      addNotification({messageKey: 'notificationRedirectingToEditTask', messageParams: {taskName: task.name}, type: 'info', link: `/project/${task.projectId}?openEditTask=${task.id}`}); 
    }
  };
  ...
  const handleViewCommentsRedirect = (taskId: string) => {
    ...
    if (task) {
      const project = getProjectById(task.projectId);
      if (project) {
        addNotification({messageKey: 'notificationRedirectingToViewComments', messageParams: { taskName: task.name }, type: 'info', link: `/project/${task.projectId}?openComments=${taskId}`}); 
      }
    }
  };
