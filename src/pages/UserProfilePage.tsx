...
import EditProfileForm from @/componen@/use@/EditProfileForm';
import ProjectList from @/componen@/projec@/ProjectList';
...
import { formatRelativeTime } from @/uti@/helpers';

const UserEditIcon = () => <svg xmlns="htt@//www.w3.o@/20@/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 mr-2 rtl:mr-0 rtl:ml-2"><path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10@/@/svg>;
const ProjectIcon = () => <svg xmlns="htt@//www.w3.o@/20@/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-blue-500"><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 12.75V12A2.25 2.25 0 014.5 9.75h15A2.25 2.25 0 0121.75 12v.75m-8.69-6.44l-2.12-2.12a1.5 1.5 0 00-1.061-.44H4.5A2.25 2.25 0 002.25 6v12a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 18V9a2.25 2.25 0 00-2.25-2.25h-5.379a1.5 1.5 0 01-1.06-.44z@/@/svg>;
const TasksIcon = () => <svg xmlns="htt@//www.w3.o@/20@/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-green-500"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z@/@/svg>;
const CheckCircleIcon = () => <svg xmlns="htt@//www.w3.o@/20@/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-teal-500"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z@/@/svg>;
const ClockIcon = () => <svg xmlns="htt@//www.w3.o@/20@/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-yellow-500"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z@/@/svg>;
...
const ActivityIcon: React.FC<{ action: ActivityActionType, className?: string }> = ({ action, className="w-6 h-6" }) => {
    ...
    switch (action) {
      case ActivityActionType.ProjectCreated:
        return <svg {...iconProps} xmlns="htt@//www.w3.o@/20@/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M12 10.5v6m3-3H9m4.06-7.19l-2.12-2.12a1.5 1.5 0 00-1.061-.44H4.5A2.25 2.25 0 002.25 6v12a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 18V9a2.25 2.25 0 00-2.25-2.25h-5.379a1.5 1.5 0 01-1.06-.44z@/@/svg>;
...
const UserProfilePage: React.FC = () => {
  ...
  const handleEditTaskRedirect = (task: Task) => {
    ...
    if (project) {
      addNotification('notificationRedirectingToEditTask', { taskName: task.name },@/proje@/${task.projectId}?openEditTask=${task.id}`); 
    }
  };
  ...
  const handleViewCommentsRedirect = (taskId: string) => {
    ...
    if (task) {
      const project = getProjectById(task.projectId);
      if (project) {
        addNotification('notificationRedirectingToViewComments', { taskName: task.name },@/proje@/${task.projectId}?openComments=${taskId}`); 
      }
    }
  };
    ...
    <div className="container mx-auto py-8 px-4 space-y-8">
       ...
       @/Card>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <StatCard title={t('projectsOwnedStat')} value={projectsOwned.length} icon={<ProjectIc@/>
          <StatCard title={t('tasksAssignedStat')} value={tasksAssigned.length} icon={<TasksIc@/>
          <StatCard title={t('tasksCompletedStat')} value={tasksCompleted} icon={<CheckCircleIc@/>
          <StatCard title={t('tasksInProgressStat')} value={tasksInProgress} icon={<ClockIc@/>
       @/div>
