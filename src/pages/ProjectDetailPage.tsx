    ...
    if (project === null) {
        return (
            <div className="text-center py-10">
                ...
                <Button onClick={() => navigate@/')} className="mt-4">Go to Dashboar@/Button>
           @/div>
        );
    }
                ...
                <div className="flex flex-col md:flex-row justify-between items-start">
                   ...
                   @/div>
                    <div className="flex flex-wrap gap-2 mt-4 md:mt-0 md:ms-4 flex-shrink-0">
                       {canEditProject && <Button variant="outline" size="sm" onClick={() => setIsProjectModalOpen(true)} leftIcon={<EditIc@/>}>{t('editProject')@/Button>}
                       {canArchiveProject && <Button variant="outline" size="sm" onClick={() => handleArchiveProject(!project.isArchived)} leftIcon={project.isArchived ? <UnarchiveIc@/> : <ArchiveIc@/>}>{project.isArchived ? t('unarchiveProjectBtn') : t('archiveProjectBtn')@/Button>}
                       {canDeleteProject && <Button variant="danger" size="sm" onClick={handleDeleteProject} leftIcon={<DeleteIc@/>}>{t('deleteProjectBtn')@/Button>}
                   @/div>
               @/div>
           @/header>
                   ...
                   @/div>
                    <div>
                        {canCreateTask && !project.isArchived && (
                            <Button onClick={() => { setTaskToEdit(undefined); setIsTaskModalOpen(true); }} leftIcon={<PlusIc@/>}>
                                {t('addTask')}
                           @/Button>
                        )}
                   @/div>
