...
import { generateId } from '@/@/uti@/helpers';
...
const ProjectIdeasPage: React.FC = () => {
  ...
  const [generatedIdeas, setGeneratedIdeas] = useState<ProjectIdea[]>([])@// These will be transient and not org-stamped until saved
  ...
  const handleGenerateIdeas = async () => {
    ...
    if (!currentUser?.organizationId) {
        setError("User organization not found. Cannot generate ideas.")@// Should not happen if logged in
        return;
    }
    setIsLoading(true);
    setError(null);
  ...
  const handleRemoveSavedIdea = (ideaId: string) => {
    const idea = savedIdeas.find(i => i.id === ideaId)@// savedIdeas is already org-scoped
  ...
  const handleCopyIdea = (idea: ProjectIdea) => {
    ...
    navigator.clipboard.writeText(ideaText).then(() => {
      ...
      addNotification('notificationIdeaCopied', {ideaName: idea.name});
...
@// getActiveProjects is already org-scoped from context
    ...
    <div key={idea.id} className="bg-white p-6 rounded-lg shadow-lg border-l-4 border-secondary hover:shadow-xl transition-shadow duration-200">
      ...
      <div className="mt-4 pt-3 border-t border-gray-100 flex flex-wrap gap-2">
        {!isSavedContext && !isIdeaSaved(idea.id) && (
          <Button onClick={() => handleSaveIdea(idea)} variant="outline" size="sm" leftIcon={<SaveIco@/>}>
            {t('ideaSaveButton')}
         @/Button>
        )}
         {isSavedContext && (
          <Button onClick={() => handleRemoveSavedIdea(idea.id)} variant="danger" size="sm" leftIcon={<TrashIco@/>}>
            {t('removeSavedIdeaButton')}
         @/Button>
        )}
        <Button onClick={() => handleCopyIdea(idea)} variant="ghost" size="sm" leftIcon={<ClipboardIco@/>}>
          {copiedIdeaId === idea.id ? t('ideaCopiedToClipboard') : t('ideaCopyButton')}
       @/Button>
        <Button onClick={() => handleCreateProjectFromIdea(idea)} variant="outline" size="sm" leftIcon={<FolderPlusIco@/>}>
          {t('ideaCreateProjectButton')}
       @/Button>
        <Button onClick={() => handleAddIdeaAsTask(idea)} variant="outline" size="sm" leftIcon={<DocumentPlusIco@/>}>
          {t('ideaAddTaskButton')}
       @/Button>
     @/div>
   @/div>
        ...
        <Modal isOpen={isProjectFormModalOpen} onClose={() => setIsProjectFormModalOpen(false)} title={t('createProject')}>
          <ProjectForm
            ...
            projectToEdit={{
              ...
              @// No 'id' is passed here, so ProjectForm treats it as a new project
            }}
        @/>
       @/Modal>
      )}
