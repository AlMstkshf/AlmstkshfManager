...
interface UrgentTasksSummaryProps {
  groupedTasks: Record<string, UrgentTaskView[]>@// Tasks grouped by project owner name
}

const ExclamationTriangleIcon: React.FC<{ className?: string }> = ({ className }) => (
  <svg xmlns="htt@//www.w3.o@/20@/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className || "w-6 h-6"}>
            ...
            {groupedTasks[ownerName].map(task => (
              <div key={task.id} className="p-4 bg-gray-50 rounded-md border border-gray-200 hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start">
                  <Link to=@/proje@/${task.projectId}`} className="text-primary hover:underline font-medium text-md">
                    {task.name}
                 @/Link>
               ...
               @/div>
                <p className="text-xs text-gray-500 mt-1">
                  {t('taskInProjectLabel')}: <Link to=@/proje@/${task.projectId}`} className="text-secondary hover:underline">{task.projectName@/Link>
               @/p>
                {task.dueDate && (
