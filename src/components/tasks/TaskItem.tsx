...
import { formatDate, getTaskPriorityPill } from '@/utils/helpers';
...
const DeleteIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
    <path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12.56 0c1.153 0 2.24.03 3.22.077m3.22-.077L10.879 3.286a1.125 1.125 0 011.07-1.071h.078a1.125 1.125 0 011.07 1.07L15.18 5.79m-3.22-.077c1.153 0 2.24.03 3.22.077" />
  </svg>
);

const CommentIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
    <path strokeLinecap="round" strokeLinejoin="round" d="M8.625 12a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375M21 12c0 4.556-3.862 8.25-8.625 8.25C7.862 20.25 4 16.556 4 12c0-4.556 3.862-8.25 8.625-8.25s8.625 3.694 8.625 8.25z" />
  </svg>
);
...
const LockClosedIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-3 h-3 text-gray-500 mr-1">
...
const TaskItem: React.FC<TaskItemProps> = ({ 
  ...
  const hasUrgentComments = comments.some(c => 
    c.isUrgent || c.sentiment === TaskCommentSentiment.Negative
  );
  ...
  return (
    <div className={`bg-white p-4 rounded-lg shadow-md border-l-4 rtl:border-l-0 rtl:border-r-4 ${getStatusBorderColor(task.status)} hover:bg-gray-50 transition-colors duration-150`}>
        ...
        <div className="flex-1">
          <div className="flex items-center">
            {isBlockedByDependency && (
              <span title={t('taskDependencyPromptMessage', { 
                nextTaskName: task.name, 
                completedTaskName: prerequisiteTask?.name || 'another task' 
              })}>
                <LockClosedIcon />
              </span>
            )}
        ...
        <div className="flex space-x-1 rtl:space-x-reverse ml-2">
          ...
          {canDeleteThisTask && (
            <Button 
              ...
              aria-label={t('deleteTaskBtn')} 
              title={t('deleteTaskBtn')}
            >
              <DeleteIcon />
            </Button>
          )}
      ...
      <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-3 text-xs">
        <div>
          ...
          <span className="text-gray-700 ml-1">
            {assignee ? assignee.name : t('unassigned')}
          </span>
        </div>
        <div>
          <span className="text-gray-500 font-medium">{t('dueDateLabel')}:</span>
          <span className="text-gray-700 ml-1">
            {formatDate(task.dueDate) || t('notAvailableShort')}
          </span>
        </div>
        <div>
          <span className="text-gray-500 font-medium">{t('priorityLabel')}:</span>
          <span className={`ml-1 px-2 py-0.5 rounded-full text-xs font-medium ${getTaskPriorityPill(task.priority)}`}>
      ...
      <div className="mt-3 pt-2 border-t border-gray-100 flex justify-end">
        ...
        >
          {hasUrgentComments ? <ExclamationCircleIcon /> : <CommentIcon />}
          <span className="ml-1">{t('viewCommentsBtn')} ({commentCount})</span>
          {hasUrgentComments && (
            <span className="sr-only">{t('commentFlaggedUrgentTooltip')}</span>
          )}
        </Button>
      </div>
    </div>
