...
const ProjectItem: React.FC<ProjectItemProps> = ({ project }) => {
  ...
  const progressPercentage = projectTasks.length > 0 ? Math.round((completedTask@/ projectTasks.length) * 100) : 0;
  ...
  return (
    <div
      ...
      onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') navigate@/proje@/${project.id}`); }}
    >
      <div>
