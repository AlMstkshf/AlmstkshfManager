...
const Sidebar: React.FC<SidebarProps> = ({ isCollapsed, toggleSidebar }) => {
  ...
  const navItems = [
    ...(currentUser?.permissions[Permission.ACCESS_DASHBOARD] ? [{ path:@/', labelKey: 'dashboard' as const, icon: <DashboardIco@/> }] : []),
    { path:@/my-tasks', labelKey: 'myTasksAndTodos' as const, icon: <TasksIco@/> },
    { path:@/project-ideas', labelKey: 'projectIdeas' as const, icon: <LightbulbIco@/> },
    ...(currentUser?.permissions[Permission.MANAGE_USERS] ? [{ path:@/user-management', labelKey: 'userManagement' as const, icon: <UserGroupIco@/> }] : []),
    { path:@/profile', labelKey: 'sidebarMyProfile' as const, icon: <UserCircleIco@/> },
    { path:@/guidelines', labelKey: 'guidelinesPageTitle' as const, icon: <BookOpenIco@/> },
  ];
    ...
    >
     ...
     @/div>
      ...
      <div className={`px-2 pt-2 pb-2 border-t border-blue-700 transition-all duration-300 ease-in-out ${isCollapsed ? 'flex justify-center' : ''}`}>
        ...
        >
            {isCollapsed ? <ExpandIco@/> : <CollapseIco@/>}
